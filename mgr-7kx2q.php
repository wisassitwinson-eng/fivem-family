...
// ตรวจสอบความพร้อมของฐานข้อมูลก่อนเริ่มทำงานหลังบ้าน
if (isset($conn) && $conn) {
...
<head>
    ...
    <style>
        body { font-family: 'Noto Sans Thai', sans-serif; background-color: #0b0c10; }
        .font-house { font-family: 'Cinzel', serif; }
        .panel { background: linear-gradient(135deg, #15171c 0%, #1c1f26 100%); border: 1px solid #2a2e37; }
        .row-item { background: #15171c; border: 1px solid #2a2e37; transition: all .2s ease; }
        .row-item:hover { border-color: #4a5568; }
        .row-item.pinned { border-color: #6b7280; box-shadow: 0 0 0 1px rgba(156, 163, 175, 0.2) inset; }
        input::placeholder { color: #6b7280; }
        .btn-save { background-color: #374151; }
        .btn-save:hover { background-color: #4b5563; }
        .btn-edit { background-color: #1f2937; color: #93c5fd; }
        .btn-edit:hover { background-color: #2c3a4f; }
        .btn-del { background-color: #3a1f24; color: #f87171; }
        .btn-del:hover { background-color: #5c2a30; }
        .pin-badge { background-color: #374151; color: #d1d5db; }
    </style>
</head>
    ...
    <div class="max-w-2xl mx-auto py-10 px-4">
        <div class="text-center mb-8">
            ...
            <div class="flex items-center justify-center gap-3 mt-1">
                <a href="index.php" class="text-gray-500 hover:text-gray-300 text-xs underline transition">← กลับไปหน้ารายชื่อสมาชิก</a>
                <span class="text-gray-700">|</span>
                <a href="mgr-7kx2q.php?logout=1" class="text-gray-500 hover:text-red-400 text-xs underline transition">ออกจากระบบ</a>
            </div>
        </div>

        <?php if ($message): ?>
            <div class="mb-6 px-4 py-3 rounded-lg text-sm <?= $messageType === 'success' ? 'bg-emerald-900/40 text-emerald-300 border border-emerald-700/50' : 'bg-red-900/40 text-red-300 border border-red-700/50' ?>">
                <?= htmlspecialchars($message) ?>
            </div>
        <?php endif; ?>

        <div class="panel rounded-xl p-5 mb-8">
            <h2 class="font-house text-lg font-bold text-gray-200 mb-4"><?= $editingMember ? '✎ แก้ไขสมาชิก: ' . htmlspecialchars($editingMember['name']) : '+ เพิ่มสมาชิกใหม่' ?></h2>
            <form method="POST" action="mgr-7kx2q.php" class="space-y-3">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="เช่น Niran Idontknow" value="<?= $editingMember ? htmlspecialchars($editingMember['name']) : '' ?>" class="w-full bg-[#0f1014] border border-gray-700 text-gray-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-gray-500">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">ลิงก์ Facebook</label>
                    <input type="url" name="facebook_url" placeholder="https://facebook.com/..." value="<?= $editingMember ? htmlspecialchars($editingMember['facebook_url']) : '' ?>" class="w-full bg-[#0f1014] border border-gray-700 text-gray-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-gray-500">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">ลิงก์รูปโปรไฟล์</label>
                    <input type="url" name="avatar_url" placeholder="https://..." value="<?= $editingMember ? htmlspecialchars($editingMember['avatar_url']) : '' ?>" class="w-full bg-[#0f1014] border border-gray-700 text-gray-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-gray-500">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">ลำดับปักหมุด (ใส่เลข 1, 2, 3... ให้อยู่บนสุดตามลำดับ / ปล่อยว่างถ้าไม่ปักหมุด)</label>
                    <input type="number" name="pin_order" placeholder="เช่น 1" value="<?= ($editingMember && $editingMember['pin_order'] !== null) ? (int)$editingMember['pin_order'] : '' ?>" class="w-full bg-[#0f1014] border border-gray-700 text-gray-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-gray-500">
                </div>
                <button type="submit" class="btn-save w-full text-gray-100 font-bold py-2.5 rounded-lg transition mt-2"><?= $editingMember ? 'บันทึกการแก้ไข' : 'บันทึกสมาชิก' ?></button>
                <?php if ($editingMember): ?>
                    <a href="mgr-7kx2q.php" class="block text-center text-gray-500 hover:text-gray-300 text-xs underline mt-2">ยกเลิกการแก้ไข</a>
                <?php endif; ?>
            </form>
        </div>
                    ...
                    <?php foreach ($members as $m): ?>
                        ...
                        <div class="row-item rounded-xl p-3 flex items-center justify-between gap-3 <?= $isPinned ? 'pinned' : '' ?>">
                            <div class="flex items-center gap-3 min-w-0">
                                <img src="<?= $avatar ?>" alt="<?= $name ?>" class="w-10 h-10 rounded-full object-cover border border-gray-700 flex-shrink-0">
                                <div class="min-w-0">
                                    <p class="font-bold text-gray-100 truncate flex items-center gap-2">
                                        <?php if ($fb): ?>
                                            <a href="<?= $fb ?>" target="_blank" rel="noopener noreferrer" class="hover:underline"><?= $name ?></a>
                                        <?php else: ?>
                                            <?= $name ?>
                                        <?php endif; ?>
                                        <?php if ($isPinned): ?>
                            ...
                            <div class="flex gap-2 flex-shrink-0">
                                <a href="mgr-7kx2q.php?edit_id=<?= $m['id'] ?>" class="btn-edit text-xs font-bold px-3 py-2 rounded-lg transition">แก้ไข</a>
                                <a href="mgr-7kx2q.php?delete_id=<?= $m['id'] ?>" onclick="return confirm('ยืนยันลบสมาชิก \"<?= $name ?>\" ออกจากตระกูล? การลบนี้ไม่สามารถย้อนกลับได้')" class="btn-del text-xs font-bold px-3 py-2 rounded-lg transition">ลบ</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
